...
## Plan: Create Author Page with Full Code
...
- Import `Author` at the top
- Add `<Route path="/author" element={<Author />} />` in the unauthenticated block (after `/cookies`)
- Add `<Route path="/author" element={<Author />} />` in the authenticated block (standalone, no `AppHeader`)

### 3. Update `src/pages/Landing.tsx`

Add an "Author" entry to `LandingNav` — in both desktop and mobile menus, as a `navigate("/author")` button placed after the section scroll links, before "Sign In".
