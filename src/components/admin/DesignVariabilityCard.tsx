...
import { Loader2, Sparkles, Palette, LayoutGrid } from "lucide-react";
import {
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={chartData.length * 40 + 20}>
                  <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 10, top: 0, bottom: 0 }}>
                    ...
                    />
                    <Bar dataKey="similarity" stackId="a" fill="hsl(var(--muted-foreground) / 0.3)" radius={[0, 0, 0, 0]}>
                      {chartData.map((_, idx) => (
                        <Cell key={idx} fill={`hsl(var(--muted-foreground) / 0.25)`} />
                      ))}
                    </Bar>
                    <Bar dataKey="variety" stackId="a" fill="hsl(var(--primary))" radius={[0, 4, 4, 0]}>
                      {chartData.map((_, idx) => (
                        <Cell key={idx} fill={`hsl(var(--primary) / 0.7)`} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
