...
- GANTT DATA FORMAT: Labels are the ROW names (e.g. customer names, project names). Use MULTIPLE datasets — one per PHASE. Each dataset.label is the phase name (e.g. "Technical Setup", "Integration", "UAT"). Each dataset.data contains [start, end] number pairs, one per label/row. Phases are color-coded by dataset index (all rows share the same phase colors). A legend is rendered automatically. Example: labels: ["Customer A", "Customer B"], datasets: [{ label: "Discovery", data: [[0, 2], [1, 3]] }, { label: "Technical Setup", data: [[2, 5], [3, 6]] }, { label: "Integration", data: [[5, 8], [6, 9]] }, { label: "Go-Live", data: [[8, 10], [9, 11]] }]. NEVER use scalar numbers — always [start, end] arrays.
