...
serve(async (req) => {
  ...
  try {
    const rateLimitResponse = await checkRateLimit(req, 'resume', 'refine-resume');
    ...
    const systemPrompt = `You are an expert resume writer and refinement specialist. You are refining an existing resume based on user feedback.
${styleContext || ''}
...
- The resume MUST still fit on exactly one printed page
...
Only flag preferences that are GENERAL (apply across documents), not task-specific. If none detected, omit this block.`;
    ...
    const messages = [
      ...
      { role: 'user', content: `Here is the current Resume HTML:\n\n${currentHtml}\n\n---\n\nPlease make this modification: ${userMessage}` },
    ];
