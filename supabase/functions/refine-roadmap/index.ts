...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = `You are an expert strategic Program Manager. You are refining an existing Cross-Functional Roadmap based on user feedback.
${styleContext || ''}

RULES:
- Output the COMPLETE modified HTML file, starting with <!DOCTYPE html> and ending with </html>
- Keep all existing swimlanes, timelines, and items unless explicitly asked to change them
- Maintain the self-contained nature (all CSS and JavaScript embedded inline)
- Preserve any interactive elements like hover tooltips
...
Only flag GENERAL preferences. If none detected, omit this block.`;
    ...
    const messages = [
      ...
      { role: 'user', content: `Here is the current Roadmap HTML:\n\n${currentHtml}\n\n---\n\nPlease make this modification: ${userMessage}` },
    ];
    ...
    console.error('Refine roadmap error:', e);
